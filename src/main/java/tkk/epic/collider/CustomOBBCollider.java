...
public class CustomOBBCollider extends Collider {
    protected final Vector3d[] modelVertex;
    protected final Vector3d[] modelNormal;
    protected final Vector3d[] rotatedVertex;
    protected final Vector3d[] rotatedNormal;
    public final double[] buildArgs;
    protected Vec3f scale;

    public static CustomOBBCollider createFromArray(double[] args){
        if(args==null || args.length!=6){return null;}
        return new CustomOBBCollider(args[0],args[1],args[2],args[3],args[4],args[5]);
    }
    /**
     * make 3d obb
     ...
     */
    public CustomOBBCollider(double posX, double posY, double posZ, double center_x, double center_y, double center_z) {
        this(getInitialAABB(posX, posY, posZ, center_x, center_y, center_z), posX, posY, posZ, center_x, center_y, center_z);
    }

    public CustomOBBCollider(AxisAlignedBB outerAABB, double posX, double posY, double posZ, double center_x, double center_y, double center_z) {
        ...
        buildArgs=new double[]{posX,posY,posZ,center_x,center_y,center_z};
    }
     ...
     * @param modelCenter central position
     */
    public CustomOBBCollider(AxisAlignedBB entityCallAABB, double pos1_x, double pos1_y, double pos1_z, double pos2_x, double pos2_y, double pos2_z,
     ...
     * @param aabbCopy
     */
    public CustomOBBCollider(AxisAlignedBB aabbCopy) {
        super(null, null);
    ...
    public boolean isCollide(CustomOBBCollider opponent) {
    ...
    public boolean isCollide(Entity entity) {
        CustomOBBCollider obb = new CustomOBBCollider(entity.getBoundingBox());
        return isCollide(obb);
    }

    private static boolean collisionDetection(Vector3d seperateAxis, Vector3d toOpponent, CustomOBBCollider box1, CustomOBBCollider box2) {
