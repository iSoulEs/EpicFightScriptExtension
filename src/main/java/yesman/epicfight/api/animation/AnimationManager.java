...
import static yesman.epicfight.api.model.JsonModelLoader.TKK_FILE_LOAD;
...
public class AnimationManager extends ReloadListener<Map<Integer, Map<Integer, StaticAnimation>>> {
	...
	private void setAnimationProperties(IResourceManager resourceManager, StaticAnimation animation) {
		...
		if (last > 0) {
			String format = String.format("%s/data%s.json", path.substring(0, last), path.substring(last));
			if(location.getNamespace().equals(TKK_FILE_LOAD)){
				try {
					File file=new File(FMLPaths.GAMEDIR.get().toFile().getCanonicalPath() + "/"+TKK_FILE_LOAD+"/"+ format);
					if(file.exists()){
						AnimationDataReader.readAndApply(animation, (new FileInputStream(file)));
					}
				} catch (IOException e) {
					e.printStackTrace();
				}
			}else {
				ResourceLocation dataLocation = new ResourceLocation(location.getNamespace(), format);
