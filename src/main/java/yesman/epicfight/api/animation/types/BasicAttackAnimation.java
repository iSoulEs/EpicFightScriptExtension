...
public class BasicAttackAnimation extends AttackAnimation {
	public BasicAttackAnimation(){super();}
	...
	public void setLinkAnimation(Pose pose1, float timeModifier, LivingEntityPatch<?> entitypatch, LinkAnimation dest) {
		...
		if(entitypatch.getTkkEnableCustom()){
			Phase phase = this.getPhaseByTime(entitypatch.getAnimator().getPlayerFor(this).getElapsedTime());
			extTime *= (float)(this.totalTime * entitypatch.getTkkModifiersAttackSpeed(entitypatch.getTkkCustomSpeed()));
			...
			extTime *= (float)(this.totalTime * entitypatch.getTkkModifiersAttackSpeed(playerpatch.getAttackSpeed(phase.hand)));
		}
