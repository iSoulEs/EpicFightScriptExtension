...
import tkk.epic.utils.TkkEpicCustomDamageSource;
...
public class DodgeAnimation extends ActionAnimation {
	...
	public DodgeAnimation(EntitySize a){
		super();
		size=a;
	}
		...
		this.stateSpectrumBlueprint.clear()
			...
			.addState(EntityState.INVULNERABILITY_PREDICATE, (damagesource) -> {
				if (damagesource instanceof EntityDamageSource && !damagesource.isExplosion() && !damagesource.isMagic() && !damagesource.isBypassInvul()) {
					if(damagesource instanceof TkkEpicCustomDamageSource && !((TkkEpicCustomDamageSource) damagesource).isCanDodge()){return false;}
					return true;
				}
				return false;
			});
