...
public class ItemCapabilityReloadListener extends JsonReloadListener {
	...
	public static CapabilityItem deserializeArmor(Item item, CompoundNBT tag) {
		...
		if (tag.contains("attributes")) {
			...
			.stunArmor(attributes.getDouble("stun_armor"))
			.moveSpeed(attributes.getDouble("moveSpeed"))
			.attackSpeed(attributes.getDouble("attackSpeed"))
			.maxStamina(attributes.getDouble("maxStamina"))
			.armor(attributes.getDouble("armor"))
			.armorToughness(attributes.getDouble("armorToughness"))
			.nametagDistance(attributes.getDouble("nametagDistance"))
			.entityGravity(attributes.getDouble("entityGravity"));
	...
	private static Map<Attribute, AttributeModifier> deserializeAttributes(CompoundNBT tag) {
		...
		//tkk
		if (tag.contains("move_speed")) {
			modifierMap.put(Attributes.MOVEMENT_SPEED, EpicFightAttributes.getMoveSpeedModifier(tag.getDouble("move_speed")));
		}
		if (tag.contains("max_stamina")) {
			modifierMap.put(EpicFightAttributes.MAX_STAMINA.get(), EpicFightAttributes.getMaxStaminaModifier(tag.getDouble("max_stamina")));
		}
		if (tag.contains("entity_gravity")) {
			modifierMap.put(ForgeMod.ENTITY_GRAVITY.get(), EpicFightAttributes.getEntityGravityModifier(tag.getDouble("entity_gravity")));
		}
		return modifierMap;
	}
