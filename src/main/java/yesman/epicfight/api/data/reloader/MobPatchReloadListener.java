...
public class MobPatchReloadListener extends JsonReloadListener {
	...
	public static class CustomMobPatchProvider extends AbstractMobPatchProvider {
		...
		//tkk
		public float tkkCustomSpeed;
		public boolean tkkEnableCustom;
		@Override
		@SuppressWarnings("rawtypes")
		public EntityPatch<?> get(Entity entity) {
	...
	public static AbstractMobPatchProvider deserializeMobPatchProvider(CompoundNBT tag, boolean clientSide) {
		...
		if (disabled) {
			...
			if (tag.contains("preset")) {
				...
				//tkk
				provider.tkkEnableCustom=tag.getBoolean("tkkEnableCustom")?tag.getBoolean("tkkEnableCustom"):false;
				provider.tkkCustomSpeed=tag.contains("tkkCustomSpeed")?tag.getFloat("tkkCustomSpeed"):1.0f;
				...
				if (!clientSide) {
					...
					if (humanoid) {
						...
						provider.attackspeed = tag.getCompound("attributes").contains("attack_speed") ? (float)tag.getCompound("attributes").getDouble("attack_speed") : 1.0F;
					} else {
	...
	public static Map<Attribute, Double> deserializeAttributes(CompoundNBT tag) {
		...
		//tkk
		attributes.put(EpicFightAttributes.OFFHAND_MAX_STRIKES.get(), (double)(tag.contains("offhand_max_strikes", 3) ? tag.getInt("offhand_max_strikes") : 1));
		attributes.put(EpicFightAttributes.OFFHAND_ARMOR_NEGATION.get(), tag.contains("offhand_armor_negation", 6) ? tag.getDouble("offhand_armor_negation") : 0.0D);
		attributes.put(EpicFightAttributes.OFFHAND_IMPACT.get(), tag.contains("offhand_impact", 6) ? tag.getDouble("offhand_impact") : 0.5D);
		if(tag.contains("offhand_attack_damage", 6)) {
			attributes.put(EpicFightAttributes.OFFHAND_ATTACK_DAMAGE.get(), tag.getDouble("offhand_attack_damage"));
		}
		return attributes;
	}
