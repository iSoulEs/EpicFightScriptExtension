...
public class JsonModelLoader {
	...
	public JsonModelLoader(IResourceManager resourceManager, ResourceLocation resourceLocation) {
		try {
			if (resourceLocation.getNamespace().equals(TKK_FILE_LOAD)) {
				File file = new File(FMLPaths.GAMEDIR.get().toFile().getCanonicalPath() + "/"+TKK_FILE_LOAD+"/");
				if (!file.exists()) {
					file.mkdirs();
				}
				BufferedInputStream inputstream = new BufferedInputStream(new FileInputStream(file+"/"+resourceLocation.getPath()));
	...
	public void loadStaticAnimationCopy(StaticAnimation animation) {
		JsonArray array = this.rootJson.get("animation").getAsJsonArray();
		boolean action = animation instanceof ActionAnimation;
		boolean attack = animation instanceof AttackAnimation;
		boolean root = true;
		...
		for (JsonElement element : array) {
			...
			if (joint == null) {
				if (name.equals("Coord") && action) {
					JsonArray jsonArray1 = keyObject.getAsJsonArray("time");
					JsonArray jsonArray2 = keyObject.getAsJsonArray("transform");
					int j = jsonArray1.size();
					int k = jsonArray2.size();
					float[] arrayOfFloat1 = new float[j];
					float[] arrayOfFloat2 = new float[k * 16];
					int m;
					for (m = 0; m < j; m++)
						arrayOfFloat1[m] = jsonArray1.get(m).getAsFloat();
					for (m = 0; m < k; m++) {
						JsonArray matrixJson = jsonArray2.get(m).getAsJsonArray();
						for (int n = 0; n < 16; n++)
							arrayOfFloat2[m * 16 + n] = matrixJson.get(n).getAsFloat();
					}
					TransformSheet transformSheet = getTransformSheet(arrayOfFloat1, arrayOfFloat2, new OpenMatrix4f(), true);
					animation.addSheet(name, transformSheet);
					//((ActionAnimation)animation).addProperty(AnimationProperty.ActionAnimationProperty.COORD, transformSheet);
					root = false;
					continue;
				}
				EpicFightMod.LOGGER.warn("[EpicFightMod] Can't find the joint " + name + " in the animation file, " + animation);
