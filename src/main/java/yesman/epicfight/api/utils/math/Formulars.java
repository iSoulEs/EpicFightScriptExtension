...
public class Formulars {
	...
	public static float getAttackSpeedPenalty(float weight, float weaponAttackSpeed, EntityPatch<?> entitypatch) {
		if (weight > WIGHT_START) {
			...
			return weaponAttackSpeed + (-0.1F * (weight / WIGHT_START) * (Math.max(weaponAttackSpeed - 0.8F, 0.0F) * 1.5F) * attenuation);
		} else { 
