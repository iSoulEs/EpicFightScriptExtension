...
public class RenderEngine {
	...
	public void renderEntityArmatureModel(LivingEntity livingEntity, LivingEntityPatch<?> entitypatch, LivingRenderer<? extends Entity, ?> renderer, IRenderTypeBuffer buffer, MatrixStack matStack, int packedLightIn, float partialTicks) {
		LivingEntity temp=!(livingEntity instanceof EntityCustomNpc)?livingEntity:(((EntityCustomNpc)livingEntity).modelData.getEntity((EntityCustomNpc) livingEntity)==null)?livingEntity:((EntityCustomNpc)livingEntity).modelData.getEntity((EntityCustomNpc) livingEntity);
	...
	public PatchedEntityRenderer getEntityRenderer(Entity entity) {
		final Entity temp=!(entity instanceof EntityCustomNpc)?entity:(((EntityCustomNpc)entity).modelData.getEntity((EntityCustomNpc) entity)==null)?entity:((EntityCustomNpc)entity).modelData.getEntity((EntityCustomNpc) entity);
		return this.entityRendererCache.get(temp.getType());
	}
	
	public boolean hasRendererFor(Entity entity) {
		final Entity temp=!(entity instanceof EntityCustomNpc)?entity:(((EntityCustomNpc)entity).modelData.getEntity((EntityCustomNpc) entity)==null)?entity:((EntityCustomNpc)entity).modelData.getEntity((EntityCustomNpc) entity);
		return this.entityRendererCache.computeIfAbsent(temp.getType(), (key) -> this.entityRendererProvider.containsKey(key) ? this.entityRendererProvider.get(temp.getType()).get() : null) != null;
	}
	...
	public void rotateCameraByMouseInput(float dx, float dy) {
		...
		this.cameraXRot = MathHelper.clamp(this.cameraXRot+f, -90.0F, 90.0F);
		this.cameraYRot += f1;
	...
	public void correctCamera(CameraSetup event, float partialTicks) {
		if (this.isPlayerRotationLocked) {
			...
			float xRot=MathHelper.rotLerp((float)partialTicks, this.cameraXRotO, this.cameraXRot);
			float yRot=MathHelper.rotLerp((float)partialTicks, this.cameraYRotO, this.cameraYRot);
