...
import tkk.epic.event.HitStunAndKnockbackEvent;
			...
			if (extendedDamageSource != null) {
				...
				if (event.getAmount() + ignoreDamage > 0.0F) {
					...
					if (hitentitypatch != null) {
						...
						if(hitentitypatch.getWeight()< Formulars.WIGHT_START){weightReduction = 1.0F;}
						...
						if (sourcePosition != null) {
							HitStunAndKnockbackEvent e=new HitStunAndKnockbackEvent(event,hitEntity,hitentitypatch,hitAnimation,extendStunTime,knockBackAmount,extendedDamageSource,extendedDamageSource.getStunType());
							if (e.hitAnimation != null) {
								...
								hitentitypatch.playAnimationSynchronized(e.hitAnimation, e.extendStunTime);
							}
							
							if (e.knockBackAmount != 0.0F) {
								hitentitypatch.knockBackEntity(((DamageSource)extendedDamageSource).getSourcePosition(), e.knockBackAmount);
							}
						}
					}
				}
