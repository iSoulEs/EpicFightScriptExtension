...
public class LiechtenauerSkill extends SpecialAttackSkill {
	...
	public void onInitiate(SkillContainer container) {
		...
		container.getExecuter().getEventListener().addEventListener(EventType.HURT_EVENT_PRE, EVENT_UUID, (event) -> {
			if (event.getAmount() > 0.0F && container.duration > 0 && this.isExecutableState(event.getPlayerPatch()) && this.canExecute(event.getPlayerPatch()) && isBlockableSource(event.getDamageSource()) && container.getExecuter().getEntityState().getLevel() <= 0) {
