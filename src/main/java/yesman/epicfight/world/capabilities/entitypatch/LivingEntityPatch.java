...
import tkk.epic.utils.TkkCustomAnimationDamageSource;
...
public abstract class LivingEntityPatch<T extends LivingEntity> extends EntityPatch<T> {
	...
	//tkk修改
	private float tkk_custom_speed=1.0F;
	private boolean tkk_enable_custom=false;

	public float getTkkModifiersAttackSpeed(float attackSpeed){
		switch (this.state.getLevel()){
			case 1:
				return this.animator.getPreDelayAttackSpeed(attackSpeed);
			case 2:
				return this.animator.getContactAttackSpeed(attackSpeed);
			case 3:
				return this.animator.getRecoveryAttackSpeed(attackSpeed);
			default:
				return attackSpeed;
		}
	...
	public void setTkkModifiersAttackSpeed(Float preDelaySet,Float preDelayAdd,Float preDelayScale,Float contactSet,Float contactAdd,Float contactScale,Float recoverySet,Float recoveryAdd,Float recoveryScale){
		...
		animator.preDelay_attackSpeedModifiers_set=preDelaySet;
		animator.preDelay_attackSpeedModifiers_add=preDelayAdd;
		animator.preDelay_attackSpeedModifiers_scale=preDelayScale;
		animator.contact_attackSpeedModifiers_set=contactSet;
		animator.contact_attackSpeedModifiers_add=contactAdd;
		animator.contact_attackSpeedModifiers_scale=contactScale;
		animator.recovery_attackSpeedModifiers_set=recoverySet;
		animator.recovery_attackSpeedModifiers_add=recoveryAdd;
		animator.recovery_attackSpeedModifiers_scale=recoveryScale;
		TkkEpicNetworkManager.sendToAllPlayerTrackingThisEntity(new SPTkkAttackSpeedModifiers(this),this.original);
	}

	public int trailTick=0;
	public boolean trailMain=false;
	public boolean trailOff=false;
	public Trail mainTrail;
	public Trail offTrail;
	public ParticleTrail mainParticleTrail;
	public ParticleTrail offParticleTrail;


	public float getTkkCustomSpeed(){
		return this.tkk_custom_speed;
	}
	public void setTkkCustomSpeed(float val){
		this.tkk_custom_speed=val;
	}
	public boolean getTkkEnableCustom(){
		return this.tkk_enable_custom;
	}
	public void setTkkEnableCustom(boolean val){
		this.tkk_enable_custom=val;
	}
	...
	public void setTrail(boolean isMainHand,boolean isValue,float x, float y, float z, float ex, float ey, float ez,int r,int g,int b,int a,int lifetime,int trailType){
		TkkEpicNetworkManager.sendToAllPlayerTrackingThisEntity(new SPTkkTrailUpdate(this.getOriginal().getId(),isMainHand,isValue,x,y,z,ex,ey,ez,r,g,b,a,lifetime,trailType),this.original);
	}
	public void setTrail(boolean isMainHand,float z,float ez,int r,int g,int b,int a,int lifetime,int trailType){
		this.setTrail(isMainHand,false,0,0,z,0,0,ez,r,g,b,a,lifetime,trailType);
	}
	public void setTrail(boolean isMainHand,float z,float ez,int r,int g,int b,int a,int lifetime){
		this.setTrail(isMainHand,z,ez,r,g,b,a,lifetime,0);
	}
	...
	public void addParticleTrail(boolean isMainHand, float x, float y, float z, float ex, float ey, float ez, boolean isValue, String particle, String args, float xSpaceBetween, float ySpaceBetween, float zSpaceBetween, float xSpeed, float ySpeed, float zSpeed, double xDist, double yDist, double zDist, int count){
		TkkEpicNetworkManager.sendToAllPlayerTrackingThisEntity(new SPParticleTrailAdd(this.getOriginal().getId(),isMainHand,x,y,z,ex,ey,ez,isValue,particle,args,xSpaceBetween,ySpaceBetween,zSpaceBetween,xSpeed,ySpeed,zSpeed,xDist,yDist,zDist,count),this.original);
	}

	public void addParticleTrail(boolean isMainHand,String particle,String arg,float x,float y,float z,float ex,float ey,float ez,float spaceBetween,float speed,double dist,int count){
		addParticleTrail(isMainHand,x,y,z,ex,ey,ez,false,particle,arg,spaceBetween,spaceBetween,spaceBetween,speed,speed,speed,dist,dist,dist,count);
	}
	public void addParticleTrail(boolean isMainHand,String particle,String arg,float z, float ez,float spaceBetween,float speed,float dist,int count){
		addParticleTrail(isMainHand,particle,arg,0,0,z,0,0,ez,spaceBetween,speed,dist,count);
	}

	public void setMainHandCollider(double posX, double posY, double posZ, double center_x, double center_y, double center_z){
		this.getAnimator().tkkCustomCollider.setMainHandCollider(posX,posY,posZ,center_x,center_y,center_z);
	}
	public void setOffHandCollider(double posX, double posY, double posZ, double center_x, double center_y, double center_z){
		this.getAnimator().tkkCustomCollider.setOffHandCollider(posX,posY,posZ,center_x,center_y,center_z);
	}
	public void setReplaceCollider(double posX, double posY, double posZ, double center_x, double center_y, double center_z){
		this.getAnimator().tkkCustomCollider.putReplaceCollider(posX,posY,posZ,center_x,center_y,center_z);
	}
	public void clearMainHandCollider(){
		this.getAnimator().tkkCustomCollider.clearMainHandCollider();
	}
	public void clearOffHandCollider(){
		this.getAnimator().tkkCustomCollider.clearOffHandCollider();
	}
	public void syncCustomCollider(){
		this.getAnimator().tkkCustomCollider.sync(this);
	}

	public TkkCustomAnimationDamageSource tkkCustomADS=new TkkCustomAnimationDamageSource();


	@Override
	public void onStartTracking(ServerPlayerEntity trackingPlayer){
		super.onStartTracking(trackingPlayer);
		EpicFightNetworkManager.sendToPlayer(new SPTkkCustomData(this.original.getId(),this.getTkkEnableCustom(),this.getTkkCustomSpeed()),trackingPlayer);
	}
	...
	protected void initAttributes() {
		...
		this.original.getAttribute(EpicFightAttributes.WEIGHT.get()).setBaseValue(40.0D);
		this.original.getAttribute(EpicFightAttributes.MAX_STRIKES.get()).setBaseValue(5.0D);
	...
	public Collider getColliderMatching(Hand hand) {
		//tkk
		return this.getAnimator().tkkCustomCollider.getHandCollider(hand,this.getAdvancedHoldingItemCapability(hand).getWeaponCollider());
	}
