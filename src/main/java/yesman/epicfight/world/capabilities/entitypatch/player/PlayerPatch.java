...
import tkk.epic.utils.TkkPlayerLivingMotionAndSpecialSkill;
...
public abstract class PlayerPatch<T extends PlayerEntity> extends LivingEntityPatch<T> {
	...
	public TkkPlayerLivingMotionAndSpecialSkill tkkPlayerLivingMotionAndSpecialSkill=new TkkPlayerLivingMotionAndSpecialSkill();
	protected float yaw;
	...
	protected void initAttributes() {
		...
		this.original.getAttribute(EpicFightAttributes.MAX_STAMINA.get()).setBaseValue(14);
	...
	public void initAnimator(ClientAnimator clientAnimator) {
		...
		clientAnimator.addLivingAnimation(LivingMotions.DRINK, Animations.FROM_118_BIPED_DRINK);
		clientAnimator.addLivingAnimation(LivingMotions.EAT, Animations.FROM_118_BIPED_EAT);
	...
	public float getAttackSpeed(Hand hand) {
		...
		if (hand == Hand.MAIN_HAND) {
			...
			baseSpeed = (float) (this.isOffhandItemValid() ? this.original.getAttributeValue(EpicFightAttributes.OFFHAND_ATTACK_SPEED.get()) : 1.6f);
		} else {
			...
			baseSpeed = (float) (this.isOffhandItemValid() ? this.original.getAttributeValue(Attributes.ATTACK_SPEED) : 1.6f);
