...
public class CapabilityItem {
	...
	public void modifyItemTooltip(ItemStack itemstack, List<ITextComponent> itemTooltip, LivingEntityPatch<?> entitypatch) {
		...
		if (attribute != null) {
			...
			Attribute max_stamina = EpicFightAttributes.MAX_STAMINA.get();
			Attribute entityGravity = ForgeMod.ENTITY_GRAVITY.get();
			...
			if (attribute.containsKey(moveSpeed)) {
				double value = attribute.get(moveSpeed).getAmount();
				if (value > 0.0D) {
					itemTooltip.add(new StringTextComponent(TextFormatting.WHITE + " +" + ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(value*100) + "% ").append(new TranslationTextComponent(moveSpeed.getDescriptionId()).withStyle(TextFormatting.WHITE)));
				}
			}
			if (attribute.containsKey(max_stamina)) {
				double value = attribute.get(max_stamina).getAmount();
				if (value > 0.0D) {
					itemTooltip.add(new StringTextComponent(TextFormatting.WHITE + " +" + ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(value) + " ").append(new TranslationTextComponent(max_stamina.getDescriptionId()).withStyle(TextFormatting.WHITE)));
				}
			}
			if (attribute.containsKey(entityGravity)) {
				double value = attribute.get(entityGravity).getAmount();
				if (value > 0.0D) {
					itemTooltip.add(new StringTextComponent(TextFormatting.WHITE + " +" + ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(value*100) + "% ").append(new TranslationTextComponent(entityGravity.getDescriptionId()).withStyle(TextFormatting.WHITE)));
				}
			}
		}
	}
	...
	public void changeWeaponSpecialSkill(PlayerPatch<?> playerpatch) {
		...
		specialSkill=playerpatch.tkkPlayerLivingMotionAndSpecialSkill.getSkill(specialSkill);
		String skillName = "";
